...
def read_channels_from_config(config_str):
    ...
    for channel in conf.keys():
        listener = ListenerFactory.create_listener_from_config(conf[channel]["listener"])
        sender = SenderFactory.create_sender_from_config(conf[channel]["sender"])
...
class ConfigFileError(IOError):
    def __init__(self, value):
        IOError.__init__(IOError, value)
        self.parameter = value
    def __str__(self):
